...
TEST_F(WebDiscoverySignatureBasenameTest, BasenameForURL) {
  ...
  for (size_t i = 0; i < 3; i++) {
    auto actual_basename =
        GenerateBasename(&profile_prefs_, server_config_, payload);
  ...
  EXPECT_FALSE(GenerateBasename(&profile_prefs_, server_config_, payload));
}
...
TEST_F(WebDiscoverySignatureBasenameTest, BasenameNotSaved) {
  ...
  for (size_t i = 0; i < 10; i++) {
    EXPECT_TRUE(GenerateBasename(&profile_prefs_, server_config_, payload));
  }
}
...
TEST_F(WebDiscoverySignatureBasenameTest, BasenameNoAction) {
  ...
  ASSERT_FALSE(GenerateBasename(&profile_prefs_, server_config_, payload));
}
