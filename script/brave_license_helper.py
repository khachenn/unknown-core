...
def AddBraveCredits(root, prune_paths, special_cases, prune_dirs,
    ...
    prune_paths.update([
        ...
        os.path.join('unknown', 'vendor', 'brave-ios'),
        os.path.join('unknown', 'vendor', 'brave_base'),
        ...
        os.path.join('unknown', 'third_party', 'rust', 'chromium_crates_io'),

        # Same for upstream
        os.path.join('third_party', 'rust', 'chromium_crates_io'),

        # Rust code written by Brave and under the same license as the browser.
        os.path.join('unknown', 'third_party', 'rust', 'adblock_cxx'),
        os.path.join('unknown', 'third_party', 'rust', 'anonymous_credentials'),
        os.path.join('unknown', 'third_party', 'rust', 'brave_news_cxx'),
        os.path.join('unknown', 'third_party', 'rust', 'brave_wallet'),
        os.path.join('unknown', 'third_party', 'rust',
                     'challenge_bypass_ristretto_cxx'),
        os.path.join('unknown', 'third_party', 'rust', 'constellation_cxx'),
        os.path.join('unknown', 'third_party', 'rust', 'document_extractor'),
        os.path.join('unknown', 'third_party', 'rust', 'json_cxx'),
        os.path.join('unknown', 'third_party', 'rust', 'filecoin_cxx'),
        os.path.join('unknown', 'third_party', 'rust', 'skus'),
        os.path.join('unknown', 'third_party', 'rust', 'skus_cxx'),
        os.path.join('unknown', 'third_party', 'rust', 'speedreader'),
        os.path.join('unknown', 'third_party', 'rust', 'speedreader_ffi'),

        # Rust crates that are references to upstream crates and should have
        # licenses in upstream //third_party/rust.
        os.path.join('unknown', 'third_party', 'rust', 'aho_corasick'),
        os.path.join('unknown', 'third_party', 'rust', 'anyhow'),
        os.path.join('unknown', 'third_party', 'rust', 'base64'),
        os.path.join('unknown', 'third_party', 'rust', 'bitflags'),
        os.path.join('unknown', 'third_party', 'rust', 'bitflags', 'v2'),
        os.path.join('unknown', 'third_party', 'rust', 'byteorder', 'v1'),
        os.path.join('unknown', 'third_party', 'rust', 'cfg_if'),
        os.path.join('unknown', 'third_party', 'rust', 'cxx'),
        os.path.join('unknown', 'third_party', 'rust', 'cxxbridge_flags'),
        os.path.join('unknown', 'third_party', 'rust', 'cxxbridge_macro'),
        os.path.join('unknown', 'third_party', 'rust', 'foldhash'),
        os.path.join('unknown', 'third_party', 'rust', 'getrandom', 'v0_2'),
        os.path.join('unknown', 'third_party', 'rust', 'hex'),
        os.path.join('unknown', 'third_party', 'rust', 'itoa', 'v1'),
        os.path.join('unknown', 'third_party', 'rust', 'lazy_static'),
        os.path.join('unknown', 'third_party', 'rust', 'libc'),
        os.path.join('unknown', 'third_party', 'rust', 'log'),
        os.path.join('unknown', 'third_party', 'rust', 'memchr'),
        os.path.join('unknown', 'third_party', 'rust', 'ppv_lite86'),
        os.path.join('unknown', 'third_party', 'rust', 'proc_macro2'),
        os.path.join('unknown', 'third_party', 'rust', 'quote'),
        os.path.join('unknown', 'third_party', 'rust', 'rand', 'v0_8'),
        os.path.join('unknown', 'third_party', 'rust', 'rand_core', 'v0_6'),
        os.path.join('unknown', 'third_party', 'rust', 'rustc_version', 'v0_4'),
        os.path.join('unknown', 'third_party', 'rust', 'regex'),
        os.path.join('unknown', 'third_party', 'rust', 'regex_automata'),
        os.path.join('unknown', 'third_party', 'rust', 'regex_syntax'),
        os.path.join('unknown', 'third_party', 'rust', 'rustversion'),
        os.path.join('unknown', 'third_party', 'rust', 'ryu'),
        os.path.join('unknown', 'third_party', 'rust', 'serde'),
        os.path.join('unknown', 'third_party', 'rust', 'serde_json'),
        os.path.join('unknown', 'third_party', 'rust', 'static_assertions'),
        os.path.join('unknown', 'third_party', 'rust', 'syn'),
        os.path.join('unknown', 'third_party', 'rust', 'unicode_ident'),
        os.path.join('unknown', 'third_party', 'rust', 'winapi'),
        os.path.join('unknown', 'third_party', 'rust', 'zerocopy', 'v0_7'),
        os.path.join('unknown', 'third_party', 'rust', 'zerocopy_derive',
                     'v0_7'),

        # Rust crates that are downloaded but not used (due to Cargo.toml
        # misconfigurations in other crates).
        os.path.join('unknown', 'third_party', 'rust', 'valuable'),
        os.path.join('unknown', 'third_party', 'rust',
                     'windows_aarch64_gnullvm'),
        os.path.join('unknown', 'third_party', 'rust', 'windows_i686_gnu'),
        os.path.join('unknown', 'third_party', 'rust', 'windows_x86_64_gnu'),
        os.path.join('unknown', 'third_party', 'rust', 'windows_x86_64_gnullvm'),
        ...
        os.path.join('unknown', 'third_party', 'android_deps'),

        # Brave overrides to third-party code, also covered by main notice.
        os.path.join('unknown', 'third_party', 'blink'),
        os.path.join('unknown', 'third_party', 'libaddressinput'),
        os.path.join('unknown', 'patches', 'third_party'),
        os.path.join('unknown', 'third_party', 'polymer'),
        os.path.join('unknown', 'third_party', 'lit'),
        os.path.join('unknown', 'third_party', 'devtools-frontend'),

        # Dependencies that are already in brave-core, and whose notices
        # therefore do not need to be repeated.
        os.path.join('unknown', 'third_party', 'updater'),
        os.path.join('unknown', 'vendor', 'omaha', 'omaha', 'third_party',
                     'chrome'),
        os.path.join('unknown', 'vendor', 'omaha', 'third_party', 'libzip'),
        os.path.join('unknown', 'vendor', 'omaha', 'third_party', 'lzma'),
        os.path.join('unknown', 'vendor', 'omaha', 'third_party', 'zlib'),

        # Dependencies already mentioned in the main breakpad notice.
        os.path.join('unknown', 'vendor', 'omaha', 'third_party', 'breakpad',
        ...
        os.path.join('unknown', 'vendor', 'depot_tools'),
        os.path.join('unknown', 'vendor', 'gn-project-generators'),
        os.path.join('unknown', 'vendor', 'omaha', 'omaha', 'scons-out'),
        os.path.join('unknown', 'third_party', 'libdmg-hfsplus'),
        os.path.join('unknown', 'tools', 'crates', 'vendor'),

        # plaster .toml file location should be skipped.
        os.path.join('unknown', 'rewrite', 'third_party'),

        # Transitive deps in brave/ui/webui/resources/wasm.
        *GetRustWorkspaceTransitiveDeps(Path('unknown/ui/webui/resources/wasm')),
    ])
    ...
    special_cases.update({
        os.path.join('unknown', 'vendor', 'bat-native-tweetnacl'): {
            "Name": "TweetNaCl",
            "URL": "https://github.com/brave-intl/bat-native-tweetnacl",
            "License": "MPL-2.0",
        },
        os.path.join('unknown', 'third_party', 'bip39wally-core-native'): {
            "Name": "libwally-core",
            "URL": "https://github.com/brave-intl/bat-native-bip39wally-core",
            "License": "MIT",
        },
        os.path.join('unknown', 'third_party', 'rust', 'futures_retry', 'v0_5'): {
            "Name": "futures-retry",
            "URL": "https://crates.io/crates/futures-retry",
            "License": "Apache-2.0",
        },
        os.path.join('unknown', 'vendor', 'brave-extension'): {
            "Name": "Brave Only Extension",
            "URL": "https://github.com/brave/brave-extension",
            "License": "MPL-2.0",
        },
        os.path.join('unknown', 'vendor', 'web-discovery-project'): {
            "Name": "Web Discovery Project",
            "URL": "https://github.com/brave/web-discovery-project",
            "License": "MPL-2.0",
        },
        os.path.join('unknown', 'vendor', 'omaha'): {
            "Name": "Omaha",
            "URL": "https://github.com/brave/omaha",
            "License": "Apache-2.0",
            "License File": ["/unknown/vendor/omaha/LICENSE.txt"],
        },
        os.path.join('unknown', 'vendor', 'omaha', 'third_party', 'breakpad'): {
            "Name": "Breakpad",
            "URL": "https://chromium.googlesource.com/breakpad/breakpad",
            "License File": [
                "/unknown/vendor/omaha/third_party/breakpad/LICENSE"
            ],
        },
        ...
        os.path.join('unknown', 'vendor', 'omaha', 'third_party', 'breakpad', 'src', 'third_party', 'musl'): {  # pylint: disable=line-too-long
            "Name": "musl",
            "URL": "https://musl.libc.org/",
            "License File": [
                "/unknown/vendor/omaha/third_party/breakpad/src/third_party/"
                "musl/COPYRIGHT"
            ],
        },
        os.path.join('unknown', 'vendor', 'omaha', 'third_party', 'googletest'): {
            "Name": "GoogleTest",
            "URL": "https://github.com/google/googletest",
            "License": "BSD",
            "License File": [
                "/unknown/vendor/omaha/third_party/googletest/LICENSE"
            ],
        },
        os.path.join('unknown', 'third_party', 'reclient_configs'): {
            "Name": "reclient-configs",
            "URL": "https://github.com/EngFlow/reclient-configs",
            "License": "Apache-2.0",
            "License File": ["/unknown/third_party/reclient_configs/src/LICENSE"],
        },
        os.path.join('unknown', 'vendor', 'python-patch'): {
            "Name": "Python Patch",
            "URL": "https://github.com/brave/python-patch",
            "License": "MIT",
            "License File": ["/unknown/vendor/python-patch/doc/LICENSE"],
        },
        os.path.join('unknown', 'vendor', 'sparkle'): {
            "Name": "Sparkle",
            "URL": "https://github.com/brave/Sparkle",
            "License": "MIT",
        },
        os.path.join('unknown', 'third_party', 'cryptography'): {
            "Name": "cryptography",
            "URL": "https://cryptography.io",
            "License": "Apache-2.0",
            "License File": ["/unknown/common/licenses/Apache-2.0"],
        },
        os.path.join('unknown', 'third_party', 'macholib'): {
            "Name": "macholib",
    ...
    additional_list += [
        os.path.join('unknown', 'resources', 'brave_new_tab_page_refresh',
                     'state', 'background_images'),
        os.path.join('unknown', 'browser', 'brave_vpn', 'win',
                     'brave_vpn_wireguard_service'),
        os.path.join('unknown', 'components', 'filecoin'),
        os.path.join('unknown', 'android', 'java', 'org', 'chromium', 'chrome',
                     'browser', 'util'),
    ]

    # Add all Android libraries since they're not directly contained
    # within a third_party directory.
    android_libs = os.path.join('unknown', 'third_party', 'android_deps', 'libs')
...
def CheckBraveMissingLicense(path, error):
    if path.startswith('unknown'):
        ...
        if path == os.path.join('unknown', 'third_party', 'rust'):
            return
        output = subprocess.check_output(
            [
                'git', 'status', '-z',
                os.path.join(os.path.relpath(path, 'unknown'), 'LICENSE')
            ],
