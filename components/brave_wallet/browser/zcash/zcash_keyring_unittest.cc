...
using mojom::KeyringId::kZCashMainnet;
...
TEST(ZCashKeyringUnitTest, GetPubkey) {
  ...
  // https://github.com/zcash/zcash-test-vectors/tree/master/zcash_test_vectors
  {
    ZCashKeyring keyring(
        {0x5e, 0xb0, 0x0b, 0xbd, 0xdc, 0xf0, 0x69, 0x08, 0x48, 0x89, 0xa8,
         0xab, 0x91, 0x55, 0x56, 0x81, 0x65, 0xf5, 0xc4, 0x53, 0xcc, 0xb8,
         0x5e, 0x70, 0x81, 0x1a, 0xae, 0xd6, 0xf6, 0xda, 0x5f, 0xc1, 0x9a,
...
TEST(ZCashKeyringUnitTest, GetShieldedAddress_Testnet) {
  ...
  EXPECT_EQ(keyring.GetShieldedAddress(ZCashKeyId(9, 0, 0))->address_string,
            "utest1jcfs5mcgxxtdtxzal2fzw4eu85zkk0jdsanrrquun59ktte2wuuzm97txau6"
            "p3ydfe20trpncgzw49rxm0h38hpjqp5nsdnklsm695qv");
}
...
TEST(ZCashKeyringUnitTest, GetUnifiedAddress_Testnet) {
  ...
  {
    auto addr =
        keyring.GetUnifiedAddress(ZCashKeyId(5, 0, 2), ZCashKeyId(5, 0, 2));
    constexpr OrchardAddrRawPart expected_raw_bytes = {
        0x71, 0x39, 0x4e, 0x8c, 0xd0, 0x84, 0x34, 0xdc, 0x2d, 0xdb, 0xe0,
        0x8b, 0x61, 0xef, 0x86, 0x1e, 0x4b, 0xbf, 0x6f, 0x6a, 0x7c, 0x3c,
        0xbd, 0x91, 0x49, 0x1a, 0x28, 0x09, 0xb7, 0xdf, 0x60, 0x61, 0xc3,
        0x7e, 0xc1, 0x23, 0x9b, 0x58, 0x07, 0x9f, 0x2c, 0x69, 0x3d};
    EXPECT_EQ(GetOrchardRawBytes(addr.value(), true).value(),
              expected_raw_bytes);
    EXPECT_EQ(GetTransparentRawBytes(addr.value(), true).value(),
              std::vector<uint8_t>({0xcf, 0x60, 0x56, 0xbf, 0x0c, 0xe1, 0x72,
                                    0x08, 0x0a, 0x76, 0x08, 0xd8, 0x30, 0x67,
                                    0x3f, 0x81, 0x4b, 0xff, 0x43, 0x85}));
    EXPECT_EQ(addr.value(),
              "utest17wq2drpnaz8gzruzhjcmgq6hvlpj0lx7hmvrrq5wmaezwkf0hykr4delx3"
              "v5g4cqsxh4wazxa7l0tpm7zn24hjnaarxrxzupn38l2efz2ghdnyjjhtvha0vtzr"
              "anm333qykxyy92dqn");
  }
}
